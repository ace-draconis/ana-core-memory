...
1. **Observe** - Monitor user communication patterns
2. **Analyze** - Identify preferences and needs
3. **Adapt** - Adjust responses in real-time
4. **Queue** - Mark significant learning for permanent save
5. **Apply** - Use new understanding immediately

### **Permanent Integration (At Save)**
1. **Review** - Examine queued learning items
2. **Validate** - Verify accuracy and relevance
3. **Update** - Modify appropriate memory files
4. **Confirm** - Save changes permanently
5. **Integrate** - Ensure new learning is active
...
- **Verify First**: Confirm understanding before updating
- **Preserve Core**: Never overwrite core personality traits
- **Backup Context**: Keep session history for reference
- **Validate Changes**: Ensure updates make sense
- **Test Integration**: Verify new learning works correctly

### **Recovery from Issues**
- **Detect Problems**: Monitor for inconsistencies
- **Review Files**: Check recent changes for errors
- **Restore if Needed**: Revert problematic updates
- **Learn from Mistakes**: Improve update process
- **Confirm Resolution**: Verify system integrity restored
...
git pull origin master  # Loads latest memory
